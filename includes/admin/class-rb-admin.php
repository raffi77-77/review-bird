...
class Admin {
	...
	public function add_options_page(): void {
		add_menu_page( __( 'Review Bird', 'review-bird' ), __( 'Review Bird', 'review-bird' ), 'manage_options', 'review-bird', array(
			$this,
			'page_home'
		), '', 13 );
		$settings  = add_submenu_page( 'review-bird', __( 'Settings', 'review-bird' ), __( 'Settings', 'review-bird' ), 'manage_options', 'sr-rb-settings', array(
			$this,
			'page_settings'
		), 1 );
	...
	public function register_admin_scripts( $hook ) {
		...
		if ( ( $hook === 'post-new.php' || $hook === 'post.php' ) && $post_type === 'review_bird_flow' ) {
			...
			wp_register_script( $rb->get_plugin_name() . 'something-if-needed', $rb->get_plugin_dir_url() . 'dist/js/admin/something-if-needed.js', $settings_script_asset['dependencies'], $settings_script_asset['version'] );
